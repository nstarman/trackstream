...
def cartesian_model(
    data: coord.CartesianRepresentation, *, lon, lat, rotation, deg: bool = True,
) -> T.Tuple:
...
def residual(
    variables: T.Sequence, data: coord.CartesianRepresentation, scalar: bool = False,
    ...
    r, lon, lat = cartesian_model(data, lon=lon, lat=lat, rotation=rotation, deg=True)
...
_minimize_defaults = dict(
    fix_origin=False, use_lmfit=None, leastsquares=False, align_v=True,
)
    ...
    **fit_kwargs,
    ...
    if use_lmfit:  # lmfit
        ...
        res, values = _fit_representation_lmfit(
            data.cartesian, x0=x0, bounds=bounds, fix_origin=fix_origin, **fit_kwargs,
        )
    ...
    best_origin = coord.UnitSphericalRepresentation(
        lon=values[1], lat=values[2],  # TODO re-add distance
    )
...
class RotatedFrameFitter(object):
    ...
    def __init__(
        ...
        bounds_args = {k: kwargs.pop(k, v) for k, v in _make_bounds_defaults.items()}
    ...
    def plot_residual(self, fitresult=None, num_rots: int = 3600, scalar: bool = True):
